...
function skyblock.feats.reset_level(player_name)
	...
	if level > 4 then return end -- prevent grinding items above level 4
	players_feat[player_name][level] = {}
	skyblock.feats.save(players_feat[player_name], player_name)
	skyblock.feats.update(player_name)
	skyblock.levels[level].init(player_name)
...
function skyblock.feats.on_placenode(pos, newnode, placer, oldnode)
	...
	or newnode.name == 'default:river_water_source' then
	  -- Check for spill priv - added by shivajiva
		if not minetest.check_player_privs(player_name, {spill=true}) then
		      ...
		      if range and (spawn==nil
			      ...
			      minetest.chat_send_player(player_name, 'You cannot place water this far from your home.')
...
for _,v in ipairs({'default:cactus', 'farming:seed_wheat', 
'farming:seed_cotton', 'default:sign_wall_wood', 'default:sign_wall_steel', 
'default:apple'}) do
	on_place(v,0);
end
...
-- register privilege to limit players that can place water beyond the anti-grief radius - added by shivajiva
minetest.register_privilege("spill","Allowed to place water without restrictions")
...
local function bucket_water_on_use(itemstack, user, pointed_thing)
	...
	if bucket.liquids[n.name] == nil then
		...
		-- Check for ocean privilege - added by shivajiva
		if not minetest.check_player_privs(player_name, {spill=true}) then
		      local spawn = skyblock.get_spawn(player_name)
		      local range = tonumber(minetest.setting_get("bucket_use_range")) or false -- how far from spawn you can use water
...
minetest.register_privilege("burn", "Allowed to place lava without restrictions")
...
local function bucket_lava_on_use(itemstack, user, pointed_thing)
	...
	if bucket.liquids[n.name] == nil then
		...
		-- Check for burn privilege - added by shivajiva
		if not minetest.check_player_privs(player_name, {burn=true}) then
		      local spawn = skyblock.get_spawn(player_name)
