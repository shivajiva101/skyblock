...
local reqdprivs = {citizen=true, interact=true}

local version = "1.1 SE"

local tpr_list = {}
local tphr_list = {}

local ghub = minetest.get_modpath("gamehub")
...
local function tpr_send(sender, receiver)
	...
	--If parameter is valid, Send teleport message and set the table.
	if not minetest.get_player_by_name(receiver) then
		return
	end
	-- check for gamehub mod
	if ghub then
	  -- check if player is using the hub
	  if gamehub.game.players[receiver] then -- deny
	    minetest.chat_send_player(sender, 'Teleport request denied, player is in the gamehub!')
	    return
	  end
	end
...
local function tphr_send(sender, receiver)
	...
	if ghub then
	  if gamehub.game.players[receiver] then
	    minetest.chat_send_player(sender, 'Teleport request denied, player is in the gamehub!')
	    return
	  end
	end
